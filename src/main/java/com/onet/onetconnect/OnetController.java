...
@ResponseBody
public  List<String> checkValid(@RequestParam("source") int source, @RequestParam("destination") int destination, @RequestParam("matrix[]") List<Integer> matrix
		) {
	...
	matrix.forEach(index-> {if(index!=-1) {
		//index=index+1;
...
public static Cell isValidSelection(int[][] matrix, int x1, int y1, int x2, int y2) {
	...
	while (queue.size() > 0) {
		...
		for (int i = 0; i < 4; i++) {
			...
			/*
			 * if(isInit) { isRowWise = (currentCell.row == adjRow) ? true: false;
			 * 
			 * }else { if(isRowWise) { isRowWise = (currentCell.row != adjRow) ? false:
			 * true; } }
			 */
			...
			if (adjRow >= 0 && adjCol >= 0 && adjRow < numberOfRows && adjCol < numberOfColumns) {
				...
				if (adjRow == x2 && adjCol == y2) {
					...
					if (currentCell.isRowWise) {
						...
						//isRowWise = (currentCell.row != adjRow) ? false : true;
						...
						//System.out.println(adjRow + "," + adjCol);
						//isRowWise = (currentCell.column != adjCol) ? false : true;
					...
					/*
					 * if (currentCell.isRowWise != isRowWise) { turns = currentCell.turn + 1;
				...
				if (matrix[adjRow][adjCol] != -1 ) {
					if (isInit) {
						...
						if (currentCell.isRowWise) {
							...
							//System.out.println("Else part:" + currentCell.isRowWise);
							//isRowWise = (currentCell.column != adjCol) ? false : true;
							if(currentCell.column != adjCol) {
								isRowWise=true;
								turns = currentCell.turn + 1;
					...
					if(turns <= 2 && matrix[adjRow][adjCol] !=1) {
						System.out.println("Added:"+adjRow + "," + adjCol+" turns:"+turns +" rowwise : "+isRowWise);
